...
import { ShoppingBag, Loader2, Check, LinkIcon, ExternalLink, MapPin, Truck, Coins, Package } from "lucide-react"
    ...
    <motion.div initial="hidden" animate="visible" variants={fadeIn}>
      <DynamicGlowCard variant="purple" className="p-6 mb-6">
        ...
        <p className="text-gray-300 mb-6">
          The final step in the WAGA Protocol workflow is token redemption. When distributors receive orders from
          customers, they redeem their tokens through the{" "}
          <code className="text-xs bg-purple-900/30 px-1 py-0.5 rounded">WAGACoffeeRedemption</code> smart contract.
          This automatically triggers the fulfillment process through integrated 3PL providers, who handle the physical
          delivery from warehouse to customer. This completes the cycle from digital tokens back to physical coffee,
          ensuring that tokens maintain their real-world value while eliminating logistics complexity for distributors.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-black/30 border border-emerald-500/20 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-400">
              Distributor submits a redemption request through the{" "}
              <code className="text-xs bg-emerald-900/30 px-1 py-0.5 rounded">WAGACoffeeRedemption</code> contract,
              specifying quantity and shipping address.
            </p>
          </div>
          <div className="bg-black/30 border border-emerald-500/20 rounded-lg p-4">
            <h4 className="font-medium text-emerald-300 mb-2">2. Token Burning</h4>
            <p className="text-sm text-gray-400">
              The specified tokens are burned via{" "}
              <code className="text-xs bg-emerald-900/30 px-1 py-0.5 rounded">burnForRedemption()</code>, removing them
              from circulation to prevent double-spending.
            </p>
          </div>
          <div className="bg-black/30 border border-emerald-500/20 rounded-lg p-4">
            <h4 className="font-medium text-emerald-300 mb-2">3. 3PL Integration</h4>
            <p className="text-sm text-gray-400">
              Smart contracts automatically notify the appropriate 3PL provider, who picks, packs, and ships the coffee
              from the warehouse.
            </p>
          </div>
          <div className="bg-black/30 border border-emerald-500/20 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-400">
              Upon delivery, the 3PL provider confirms completion, and the redemption is marked as fulfilled on the
              blockchain.
            </p>
          </div>
        </div>
      </DynamicGlowCard>
      <DynamicGlowCard variant="purple" className="p-6 mt-6">
        <h3 className="text-lg font-medium text-purple-300 mb-4">3PL Integration Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h4 className="font-medium text-purple-300 mb-3">Smart Contract Integration</h4>
            <div className="bg-black/40 rounded-lg p-4">
              <p className="text-sm text-gray-400 mb-3">
                The redemption process is handled by the{" "}
                <code className="text-xs bg-purple-900/30 px-1 py-0.5 rounded">WAGACoffeeRedemption</code> contract,
                which coordinates with 3PL providers through Chainlink Functions.
              </p>
              <div className="text-xs text-gray-500 font-mono bg-black/30 p-3 rounded overflow-auto max-h-40">
                <div className="text-purple-400">// Request redemption of coffee tokens</div>
                <div>function requestRedemption(</div>
                <div> uint256 batchId,</div>
                <div> uint256 quantity,</div>
                <div> string memory deliveryAddress</div>
                <div>)</div>
                <div className="text-purple-400">// Update redemption status as it progresses</div>
                <div>function updateRedemptionStatus(</div>
                <div> uint256 redemptionId,</div>
                <div> RedemptionStatus status</div>
                <div>)</div>
              </div>
            </div>
          </div>
          ...
          <div>
            <h4 className="font-medium text-purple-300 mb-3">Fulfillment Workflow</h4>
            <div className="space-y-3">
              <div className="flex items-start bg-black/40 p-3 rounded-lg">
                <div className="bg-purple-900/30 p-2 rounded-full mr-3 flex-shrink-0">
                  <Truck className="h-4 w-4 text-purple-400" />
                </div>
                <div>
                  <h5 className="text-sm font-medium text-purple-300">Automated Dispatch</h5>
                  <p className="text-xs text-gray-400">
                    When tokens are redeemed, the system automatically selects the optimal 3PL provider based on
                    location, cost, and service level.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start bg-black/40 p-3 rounded-lg">
                ...
                <div>
                  <h5 className="text-sm font-medium text-purple-300">Warehouse Instructions</h5>
                  <p className="text-xs text-gray-400">
                    Detailed picking instructions are sent to the warehouse, including batch information, packaging
                    requirements, and shipping labels.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start bg-black/40 p-3 rounded-lg">
                ...
                <div>
                  <h5 className="text-sm font-medium text-purple-300">Blockchain Verification</h5>
                  <p className="text-xs text-gray-400">
                    Each step of the fulfillment process is recorded on the blockchain, providing end-to-end
                    traceability from producer to consumer.
                  </p>
                </div>
              </div>
            </div>
