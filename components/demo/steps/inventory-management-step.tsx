  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={fadeIn}>
      <DynamicGlowCard variant="purple" className="p-6 mb-6">
        ...
        <div className="bg-black/30 border border-purple-500/20 rounded-lg p-4 mb-6">
          <h3 className="text-lg font-medium text-purple-300 mb-3">Digital Token Inventory Management</h3>
          <p className="text-gray-300 mb-4">
            Distributors manage their inventory entirely through tokens on the blockchain. Each token represents a claim
            on physical coffee stored in secure warehouses. This vendor-managed inventory system eliminates the need for
            distributors to handle physical product.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-black/40 rounded-lg p-4">
              <h4 className="font-medium text-purple-300 mb-2">Smart Contract Integration</h4>
              <p className="text-sm text-gray-400 mb-3">
                The WAGACoffeeToken and WAGAInventoryManager contracts work together to maintain accurate inventory
                records on the blockchain.
              </p>
              <div className="text-xs text-gray-500 font-mono bg-black/30 p-3 rounded overflow-auto max-h-32">
                <div className="text-purple-400">// Distributor token balance represents inventory</div>
                <div>function balanceOf(address distributor, uint256 batchId)</div>
                <div className="text-purple-400">// Check if batch is verified before distribution</div>
                <div>function isBatchVerified(uint256 batchId) returns (bool)</div>
                <div className="text-purple-400">// Update inventory when tokens are redeemed</div>
                <div>function burnForRedemption(address, uint256 batchId, uint256 amount)</div>
              </div>
            </div>

            <div className="bg-black/40 rounded-lg p-4">
              <h4 className="font-medium text-purple-300 mb-2">Automated Inventory Checks</h4>
              <p className="text-sm text-gray-400 mb-3">
                Chainlink Automation regularly performs inventory checks to ensure token balances match physical
                inventory.
              </p>
              <ul className="space-y-2 text-xs text-gray-400">
                <li className="flex items-start">
                  <div className="bg-purple-900/30 p-1 rounded-full mr-2 mt-0.5 flex-shrink-0">
                    <span className="text-xs text-purple-300">✓</span>
                  </div>
                  <span>Low inventory warnings when token supply drops below threshold</span>
                </li>
                <li className="flex items-start">
                  <div className="bg-purple-900/30 p-1 rounded-full mr-2 mt-0.5 flex-shrink-0">
                    <span className="text-xs text-purple-300">✓</span>
                  </div>
                  <span>Expiry date monitoring to prevent distribution of expired product</span>
                </li>
                <li className="flex items-start">
                  <div className="bg-purple-900/30 p-1 rounded-full mr-2 mt-0.5 flex-shrink-0">
                    <span className="text-xs text-purple-300">✓</span>
                  </div>
                  <span>Verification of physical reserves through oracle networks</span>
                </li>
              </ul>
            </div>
          </div>
