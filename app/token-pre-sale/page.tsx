...
export default function TokenPreSalePage() {
  ...
  const handleApproveAndBuyWithUsdc = async () => {
    if (!address) {
      openConnectModal();
      return;
    }
    ...
    const needsApproval = currentAllowance < usdcValue;
    ...
    if (needsApproval) {
      try {
        ...
        await writeApproveContract({
          ...
          args: [TOKEN_SHOP_CONTRACT_ADDRESS, approvalAmount],
        });
      } catch (error) {
    ...
    const checkAllowanceInterval = setInterval(async () => {
      const refetched = await refetchUsdcAllowance();
      const newAllowance = refetched.data
        ? parseFloat(formatUnits(refetched.data, 6))
        : 0;

      if (newAllowance >= usdcValue) {
        clearInterval(checkAllowanceInterval);
        try {
          await writeShopContract({
            address: TOKEN_SHOP_CONTRACT_ADDRESS,
            abi: wagmiContractConfig.abi,
            ...
            >
              ...
              >
                ...
                <TokenPurchaseTabs
                  ...
                  onBuyWithUsdc={handleApproveAndBuyWithUsdc}
